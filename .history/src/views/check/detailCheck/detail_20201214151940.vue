...
export default {
  ...
  methods: {
    ...
    enrol() {
    checkPass(id=this.detailForm.id,adminId=this.$store.getters.token,checkState=2).then(successResponse => {
    ...
    Pass(){
    checkPass(id=this.detailForm.id,adminId=this.$store.getters.token,checkState=1).then(successResponse => {
          if (successResponse.data.code === 0) {
          } else {
            this.$message({
              showClose: true,
